...
$(function() {
    ...
    /*$('.dnlist').dropload({
        scrollArea : window,
        loadDownFn : function(me){
		var page = parseInt(ztfmmore.attr("data-page")) + 1;
		var pageCount = count;
		$.get('/apis/ajaxzt.html', {page: page,tagid: cid,tempid:tmpidnum,line: linenum,ajax: 1,gid:gidnum,order: ''},function(data) {
			if (page < pageCount && data) {					
				//setTimeout(function(){
                        $('#applist').append(data);
						ztfmmore.attr("data-page",page);
                        // 每次数据加载完，必须重置
                        me.resetload();
                   // },1000);	
    ...
    /*$('.dnlist').dropload({
        scrollArea : window,
        loadDownFn : function(me){
		var page = parseInt(ztclassmore.attr("data-page")) + 1;
		var pageCount = count;
		$.get('/apis/ajaxzt.html', {page: page,tagid: cid,tempid:tmpidnum,line: linenum,ajax: 1,orderby: order},function(data) {
			if (page < pageCount && data) {					
			//	setTimeout(function(){
                        $('#applist').append(data);
						ztclassmore.attr("data-page",page);
                        // 每次数据加载完，必须重置
                        me.resetload();
                  //  },1000);	
